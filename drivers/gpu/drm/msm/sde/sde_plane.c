...
static const struct sde_csc_cfg sde_identity_csc_cfg = {
	...
	{ 0, 0, 0 },
	/* unsigned clamp */
	{ 0, CSC_8BIT_LIMIT, 0, CSC_8BIT_LIMIT, 0, CSC_8BIT_LIMIT },
	{ 0, CSC_8BIT_LIMIT, 0, CSC_8BIT_LIMIT, 0, CSC_8BIT_LIMIT },
};
					  ...
					  const struct sde_csc_cfg *csc_cfg)
{
	...
	memset(&psde->csc_pcc_cfg.csc_mv, 0, sizeof(psde->csc_pcc_cfg.csc_mv));
	...
	for (i = 0; i < 3; i++) {
		for (j = 0; j < 3; j++) {
			...
			for (u = 0; u < 3; u++) {
				...
				s64 v = csc_to_signed(csc_cfg->csc_mv[uj]);
