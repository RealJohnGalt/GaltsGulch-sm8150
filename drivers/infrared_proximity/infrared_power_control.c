...
static ssize_t infrared_power_enable_store(struct device *pdev,
		struct device_attribute *attr, const char *buff, size_t count)
{
	...
	INFRARED_LOG("infrared_power_enable : %d, enable : %d",
			g_infrared_state->infrared_power_enable, enable);

	return count;
}

static ssize_t infrared_power_enable_show(struct device *dev,
...
static ssize_t infrared_shut_down_state_store(struct device *pdev,
		struct device_attribute *attr, const char *buff, size_t count)
{
...
static ssize_t infrared_shut_down_state_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	if (g_infrared_state == NULL) {
		INFRARED_ERR("g_infrared_state == NULL\n");
...
static ssize_t infrared_shut_down_state2_store(struct device *pdev,
		struct device_attribute *attr, const char *buff, size_t count)
{
	unsigned long infrared_shut_down_state = 0;
	...
	if (sscanf(buff, "%lu", &infrared_shut_down_state) == 1) {
		g_infrared_state->infrared_shutdown_state2 =
				infrared_shut_down_state;
		INFRARED_LOG(
			"would set infrared_shut_down_state2 : %d",
			g_infrared_state->infrared_shutdown_state2);
	}
	INFRARED_LOG("infrared_shut_down_state : %d",
						infrared_shut_down_state);
...
static void infrared_irq_check_work_func(struct work_struct *work)
{
	...
	struct oneplus_infrared_state *state = container_of(dwork,
		struct oneplus_infrared_state, infrared_irq_check_work);

	if (state == NULL) {
		INFRARED_ERR("%s error, state == NULL", __func__);
		return;
	}
...
static irqreturn_t oneplus_infrared_detect_handler (int irq, void *dev_id)
{
	...
	if (g_infrared_state->irq_times == 1)
		mod_delayed_work(system_highpri_wq,
			&g_infrared_state->infrared_irq_check_work,
							msecs_to_jiffies(1));
...
static void oneplus_infrared_interrupt_register(
				struct oneplus_infrared_state *state)
{
	...
	state->shutdown_state =
		pinctrl_lookup_state(state->pctrl, "infrared_input");
	...
	if (!gpio_is_valid(state->infrared_gpio)) {
		...
		if (err)
			INFRARED_LOG(
			"request infrared_gpio gpio failed, err : %d\n", err);

		err = gpio_direction_input(state->infrared_gpio);
		msleep(50);
		state->infrared_irq = gpio_to_irq(state->infrared_gpio);

		if (request_irq(state->infrared_irq,
			&oneplus_infrared_detect_handler,
...
static int infrared_platform_probe(struct platform_device *pdev)
{
	...
	if (g_infrared_state == NULL) {
		p_infrared_state = kzalloc(
			sizeof(struct oneplus_infrared_state), GFP_KERNEL);
		if (!p_infrared_state) {
	...
	p_infrared_state->vdd =
		devm_regulator_get(p_infrared_state->dev, "vdd");
	if (IS_ERR(p_infrared_state->vdd)) {
		...
		regulator_count_voltages(p_infrared_state->vdd) > 0) {
		err = regulator_set_voltage(p_infrared_state->vdd,
					2960000, 2960000);
		if (err)
			INFRARED_ERR(
			"regulator set vdd voltage failed, err : %d\n", err);
		err = regulator_set_load(p_infrared_state->vdd, 20000);
		if (err)
			INFRARED_ERR(
			"regulator set vdd load failed, err : %d\n", err);
	}
	...
	INIT_DELAYED_WORK(&p_infrared_state->infrared_irq_check_work,
						infrared_irq_check_work_func);
...
static int infrared_platform_suspend(struct platform_device *pdev,
						pm_message_t state)
{
	INFRARED_LOG("call\n");
